...
def product_show(auth_head, page, items_page):
    ...
    if auth_code['time'] <= time.time():
        return "time expired login again"
    if auth_code['role'] == "owner":
        prods = Product.query.filter_by(owner_id=auth_code["user_id"])
        ls = []
        for each in prods:
            row = {}
            row['prod_id'] = each.id
            ...
            ls.append(row)
        return ls[(page-1)*items_page:page*items_page]
    ...
    for each in prods:
        ...
        row["name"] = each.name
        row["price"] = each.price
        meta = MetaProduct.query.filter_by(product_id=each.id).first()
        if meta!=None:
            row["image1"] = meta.image1_url
...
def prod_search(auth_head,cat ):
    auth_code = jwt.decode(auth_head, current_app.config["SECRET_KEY"])
    if auth_code['time'] <= time.time():
        return "time expired login again"
    if auth_code['role'] == "owner":
        return "user not allowed "
    query = "select p.name, p.price,p.id from products as p join product_categories as pc on pc.product_id = p.id join categories as c  on c.id = pc.category_id where c.name like '%{}%' ;".format(cat)
...
def cart_show(auth_head):
    ...
    if auth_code['role'] != "user":
        return "user not allowed "
    user_id = auth_code['user_id']
    query = "select p.name as name, c.quantity as quantity from products as p join cart as c on c.product_id = p.id where c.user_id = {};".format(user_id)
    wish = db.session.execute(query)
    ls = []
    for each in wish:
        row = {}
